...
it('creates a project', function () {
    $this->actingAs(User::factory()->create())
        ->post('/settings/projects', ['key' => 'BILLR', 'name' => 'Billr'])
...
it('creates a project with a color', function () {
    $this->actingAs(User::factory()->create())
        ->post('/settings/projects', ['key' => 'SHOP', 'name' => 'Shop', 'color' => '#378add'])
        ->assertRedirect(route('projects.index'));
...
it('rejects an invalid color', function () {
    $this->actingAs(User::factory()->create())
        ->post('/settings/projects', ['key' => 'SHOP', 'name' => 'Shop', 'color' => 'blue'])
        ->assertSessionHasErrors('color');
});

it('rejects a project key that is not 2-10 uppercase letters', function () {
    $this->actingAs(User::factory()->create())
        ->post('/settings/projects', ['key' => 'thi', 'name' => 'Thijssen Software'])
...
it('rejects a duplicate project key', function () {
    ...
    $this->actingAs(User::factory()->create())
        ->post('/settings/projects', ['key' => 'THI', 'name' => 'Duplicate'])
        ->assertSessionHasErrors('key');
});
...
it('allows renaming a project with no issues, including its key', function () {
    ...
    $this->actingAs(User::factory()->create())
        ->patch("/settings/projects/{$team->id}", ['key' => 'NEW', 'name' => 'New name'])
        ->assertRedirect(route('projects.index'));
...
it('prohibits changing the key once a project has issues', function () {
    ...
    $this->actingAs(User::factory()->create())
        ->patch("/settings/projects/{$team->id}", ['key' => 'CHANGED', 'name' => 'Thijssen Software'])
        ->assertSessionHasErrors('key');
...
it('still allows renaming a locked project as long as the key is omitted', function () {
    ...
    $this->actingAs(User::factory()->create())
        ->patch("/settings/projects/{$team->id}", ['name' => 'New name'])
        ->assertRedirect(route('projects.index'));
